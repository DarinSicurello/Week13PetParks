...
public class ParkService {
    ...
    private Contributor findOrCreateContributor(Long contributorId) {
        ...
        // If contributorId is null, create a new contributor, otherwise find the existing one
        if (Objects.isNull(contributorId) || contributorId == 0) {
    ...
    private Contributor findContributorById(Long contributorId) {
        return contributorDao.findById(contributorId)
                .orElseThrow(() -> new NoSuchElementException(
                        "Contributor with ID=" + contributorId + " was not found.")); // Fixed typo
    }
}
